...
(defun update-game (world)
  ...
  (cond ((eq (gethash 'frame world) 0)
	 (let ((newrows ())
	       (newrow()))
	   (setf newrows (cons (nth 50 (gethash 'cells world)) newrows)) ;; last row doesn't change
	   (loop for i from (+ (gethash 'unscaled-height world) 1) downto 1 do
		(setf newrow (cons (nth 66 (nth i (gethash 'cells world))) newrow)) ;; last cell in row doesn't change
		(loop for j from (+ (gethash 'unscaled-width world) 1) downto 1 do
		     (cond ((and (eq (nth (+ j 1) (nth (+ i 1) (gethash 'cells world))) 0)
